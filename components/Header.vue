    ...
    <nav class="container mx-auto px-4 sm:px-6 py-3 flex flex-wrap justify-between items-center">
      <div class="flex justify-between items-center w-full sm:w-auto">
        ...
        <div class="flex items-center space-x-2 sm:space-x-4">
          <button class="sm:hidden p-2" @click="openLoginMenu" title="Ingresar">
          ...
          <button class="sm:hidden p-2" @click="openRegisterMenu" title="Registrarse">
            <svg class="w-5 h-5 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      ...
      <ul :class="{'hidden': !isMenuOpen, 'flex': isMenuOpen, 'sm:flex': true, 'flex-col': true, 'sm:flex-row': true, 'space-y-4': true, 'sm:space-y-0': true, 'sm:space-x-4': true, 'mt-4': true, 'sm:mt-0': true, 'w-full': true, 'sm:w-auto': true}" v-if="!user">
        <li>
      ...
      <div class="hidden sm:flex items-center space-x-4" v-if="!user">
        <button class="bg-primary text-quaternary px-4 py-2 rounded hover:bg-tertiary transition-colors" @click="openLoginMenu">
          Ingresar
        </button>
      ...
      <div :class="{'hidden': !isMenuOpen, 'flex': isMenuOpen, 'sm:flex': true, 'items-center': true, 'justify-between': true, 'w-full': true, 'sm:w-auto': true}" v-if="user">
        <ul :class="{'hidden': !isMenuOpen, 'flex': isMenuOpen, 'sm:flex': true, 'flex-col': true, 'sm:flex-row': true, 'space-y-4': true, 'sm:space-y-0': true, 'sm:space-x-4': true, 'mt-4': true, 'sm:mt-0': true, 'w-full': true, 'sm:w-auto': true}">
          <li>
        ...
        <button class="bg-primary text-quaternary px-4 py-2 rounded hover:bg-tertiary transition-colors" @click="logout">
          Cerrar sesión
        </button>
      </div>
    </nav>
